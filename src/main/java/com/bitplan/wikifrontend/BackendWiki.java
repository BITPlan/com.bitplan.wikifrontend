...
public class BackendWiki extends Mediawiki {
  ...
  private SiteInfo siteinfo;
	...
	public void setWikiProperties(Properties props) {
		...
		this.setFrame(props.getProperty("wiki.frontend.frame","MediaWiki:Frame.rythm"));
	}
  ...
  /**
	...
	public boolean checkCategory(String content, String categoryName)
		...
		if (content != null) {
			...
			for (Matcher m = Pattern.compile(pattern, Pattern.MULTILINE).matcher(
					content); m.find();) {
	...
	/**
	 * frame the given html with the given title using the rythm template for this BackendWiki
	 * @param title
	 * @param html 
	 * @return
	 * @throws Exception
